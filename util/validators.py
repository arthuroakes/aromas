...
def is_in_range(
    self: int | float, self_name: str, low: int | float, high: int | float, errors: dict
) -> bool:
    if low <= self <= high:
        return True
    else:
...
# def is_categoria_existe(self: str, self_name: str, errors: dict) -> bool:
#     if re.match(r"^\d{5}-\d{3}$", self) is not None:
#         return True
#     else:
#         add_error(self_name, f"A categoria com o nome já existe.", errors)
#         return False
...
def is_greater_than(
    self: int | float, self_name: str, value: int | float, errors: dict
) -> bool:
    if self > value:
        return True
    else:
