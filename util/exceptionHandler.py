...
def configurar(app: FastAPI):
  ...
  async def request_validation_error(request: Request,
                                     exc: RequestValidationError):
    erros = {}
    errors = exc.errors()
    for erro in errors:
      campo, mensagem = erro["loc"], erro["msg"]
      campo_isolado = campo[1:] if campo[0] in ("body", "query",
                                                "path") else campo
