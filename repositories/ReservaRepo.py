...
class ReservaRepo:
  ...
  def obterReserva(cls, pagina: int, tamanhoPagina: int) -> List[Reserva]:
      ...
      sql = "SELECT idReserva, usuario.nome, reserva.idMesa, mesa.nome as nomeMesa, dataHoraCadastro, dataReserva, horaReserva, qtdPessoas FROM reserva INNER JOIN mesa ON reserva.idMesa = mesa.idMesa INNER JOIN usuario ON reserva.idCliente = usuario.idUsuario ORDER BY idReserva LIMIT ?, ?"
      conn = Database.createConnection()
      cursor = conn.cursor()
