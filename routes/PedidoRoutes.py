...
async def carrinho(
    ...
    if usuario and usuario.cliente:
        if idProduto:
            ...
            if produto:
                pedido: Pedido = PedidoRepo.getPedidoByClienteByStatus(usuario.idUsuario, "carrinho")
                if not pedido:
...
async def sobrenos(request: Request, usuario: Usuario = Depends(validar_usuario_logado)):
    ...
    if usuario and usuario.cliente:
        qtdeItensCarrinho = ItemRepo.getCountCartItemsFromUser(usuario.idUsuario)
    return templates.TemplateResponse("Pedido/sucessoPedido.html", {"request": request, "usuario": usuario, "qtdeItensCarrinho": qtdeItensCarrinho}) 


@router.get("/andamento", response_class=HTMLResponse)
async def pedido_andamento(
    request: Request, usuario: Usuario = Depends(validar_usuario_logado)
):
    pedido = PedidoRepo.getPedidoByCliente(usuario.idUsuario) 
    if pedido:
        PedidoRepo.getStatusByPedido(pedido.idPedido)
        endereco = EnderecoRepo.obterEnderecoPorId(pedido.idEndereco)  
        cliente = ClienteRepo.obterPorId(usuario.idUsuario) 
        codigo = PedidoRepo.gerar_sequencia_numeros()  
        qtdeItensCarrinho = 0
        if usuario and usuario.cliente: 
            qtdeItensCarrinho = ItemRepo.getCountCartItemsFromUser(usuario.idUsuario)
    return templates.TemplateResponse("Pedido/andamentoped.html", {"request": request, "usuario": usuario, "cliente": cliente, "pedido": pedido, "endereco": endereco, "qtdeItensCarrinho": qtdeItensCarrinho, "codigo": codigo})
...
async def get_motivo_cancelamento(
    ...
    if usuario and usuario.cliente:  
        qtdeItensCarrinho = ItemRepo.getCountCartItemsFromUser(usuario.idUsuario)
    return templates.TemplateResponse(
        "Pedido/ped_cancel.html", {"request": request, "usuario": usuario, "qtdeItensCarrinho": qtdeItensCarrinho}
    ) 

@router.get("/pedidocancelado", response_class=HTMLResponse)
async def pedidocancelado(request: Request, usuario: Usuario = Depends(validar_usuario_logado)):
    qtdeItensCarrinho = 0
    if usuario and usuario.cliente:
        qtdeItensCarrinho = ItemRepo.getCountCartItemsFromUser(usuario.idUsuario)
    return templates.TemplateResponse(
        "Pedido/pedidocancelado.html", {"request": request, "usuario": usuario, "qtdeItensCarrinho": qtdeItensCarrinho}
    ) 
...
async def get_excluir_pedido(
    ...
    if not pedido:
        return templates.TemplateResponse("Pedido/pedidonaoencontrado.html", {"request": request}, status_code=404)
    return templates.TemplateResponse(
...
async def post_excluir_pedido( 
    ...
    if PedidoRepo.delete(idPedido):
        if usuario.funcionario:
            return RedirectResponse("/pedido/listagempedidos", status_code=status.HTTP_303_SEE_OTHER)
        else: 
            return RedirectResponse("/pedido/listagemtodospedidos", status_code=status.HTTP_303_SEE_OTHER)
    else:
